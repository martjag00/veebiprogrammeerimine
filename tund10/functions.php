<?php
 ...
 function getuserprofile($userId){
	...
	$stmt = $mysqli->prepare("SELECT description, bgcolor, txtcolor FROM vpuserprofiles WHERE id = ?");
	echo $mysqli -> error;
...
function setuserprofile($userId, $description, $foreground, $background){
	$notice = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $mysqli->prepare("REPLACE INTO vpuserprofiles (userid, description, bgcolor, txtcolor) VALUES (?, ?, ?, ?)"); 
	echo $mysqli -> error;
